...
double     **pi, *alpha, *varphi, *y, *varphiVio;
double        *x1;
double     *w;
...
void main(int argc, char *argv[])
{
	...
	for (numSample = 1; numSample < MaxNumInst; numSample++){
		...
		M=0;
		///// To run the codes in Visual Studio, you should use clock() structure for time instead of gettimeofday(&startTotal, NULL) structure.
		//startTemp = clock();
		for (h = 0; h < A + K; h++){
			M += vc[h];
		}
