...
extern double     **pi, *alpha, *varphi, *y, *varphiVio, **pi_pareto, *alpha_pareto, *piVio_pareto, *alphaVio_pareto;
extern double        *x1;
...
void read_instance(const char *name)
{
	...
	if (fscanf(in, "%d %d %d %d", &N, &A, &K, &S) != 4){
	...
	//Cost of transmitting commodity k ∈ K through arc h ∈ A. In the paper, we show this parameter with r^k_h
	for (h = 0; h < A; h++){
		fscanf(in, "%lf", &vc[h]);
	}
	for (h = 0; h < A; h++){//Capacity of arc h ∈ A. In the paper, we show this parameter with q
		fscanf(in, "%lf", &c[h]);
	}
	for (h = 0; h < A; h++){//Installation cost of arc h ∈ A. In the paper, we show this parameter with c
		fscanf(in, "%lf", &f[h]);
	}
	for (k = 0; k < K; k++){
	...
	//This is the profit for a maximization problem. In the paper, we use the cost minimization problem.
	for (h = 0; h < A; h++){
		for (k = 0; k < K; k++){
...
void Initialize_memory(void)
{
	...
	varphiVio = create_double_vector(A + K);
	piVio = create_double_vector(N * K);
	alphaVio = create_double_vector(A + K);

	piVio_pareto = create_double_vector(N * K);
	alphaVio_pareto = create_double_vector(A + K);

	w_pareto = create_double_vector((A + K)*max_iter);
	alpha_pareto = create_double_vector((A + K)*S*max_iter);
	pi_pareto = create_double_matrix(N*K*S, max_iter);
}
