...
extern double     **pi, *alpha, *varphi, *y, **pi_pareto, *alpha_pareto, *piVio_pareto, *alphaVio_pareto, *varphi_pareto;
extern double        *x1;
...
double solve_MasterProblem(void)
{
	...
	for (pareto_iter = 3; pareto_iter < (iter + 1); pareto_iter++){
		...
		for (s = 0; s < S; s++){
			...
			for (h = 0; h < (A + K); h++){
				matind[index] = pos_w[h];
				matval[index++] = c[h] * (alpha_pareto[(h*S + s) + (pareto_iter*(A + K)*S)] - varphi_pareto[(h*S + s) + (ii*(A + K)*S)]);
			}
		}
		status = CPXaddrows(envPre, lpPre, 0, index1, index, rhs, sense, matbeg, matind, matval, NULL, NULL);
		if (status)
