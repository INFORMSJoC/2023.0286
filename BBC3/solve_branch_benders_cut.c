...
extern double     **pi, *alpha, *varphi, *y, **pi_pareto, *alpha_pareto, *varphi_pareto, *varphiVio;
extern double        *x1;
extern double        *w;
...
double solve_branch_benders_cut(void)
{
	...
	//Add constraint 3: flow on each arc should be less than the capacity of that arc
	numrows = A + K;
	numnz = K * (A + K) + A + K;
	d_vector(&rhs, numrows, "open_cplex:2");
	c_vector(&sense, numrows, "open_cplex:3");
	...
	//Add constraint 37: theta >= theta 0
	numrows = S;
	numnz = S;
	d_vector(&rhs, numrows, "open_cplex:2");
	c_vector(&sense, numrows, "open_cplex:3");
	...
	//////pareto cut//////
	for (pareto_iter = 3; pareto_iter < (iter); pareto_iter++){
		for (s = 0; s < S; s++){
			temp1[s] = 0;
		}
		for (k = 0; k < K; k++){
		...
		//// add opimality cut 36 obtained by solving PSP_s (40)/////////
		numrows = S*(iter + 1);
		numnz = S + S*(A + K);
		d_vector(&rhs, numrows, "open_cplex:2");
		c_vector(&sense, numrows, "open_cplex:3");
		...
		for (s = 0; s < S; s++){
			...
			for (h = 0; h < (A + K); h++){
				matind[index] = pos_w[h];
				matval[index++] = c[h] * (alpha_pareto[(h*S + s) + (pareto_iter*(A + K)*S)]- varphi_pareto[(h*S + s) + (pareto_iter*(A + K)*S)]);
			}
		}
	...
	for (ii = 0; ii < iter; ii++){
		...
		for (s = 0; s < S; s++){
			...
			for (h = 0; h < A + K; h++){
				matind[index] = pos_w[h];
				matval[index++] = c[h] * (alpha[(h*S + s) + (ii*(A + K)*S)] - varphi[(h*S + s) + (ii*(A + K)*S)]);
			}
		}
		finishOptimalityCuts = finishOptimalityCuts + index1;
		status = CPXaddrows(env, lp, 0, index1, index, rhs, sense, matbeg, matind, matval, NULL, NULL);
	...
	/// Variable fixing presented in Section 5.4.2 of the paper
	for (h = 0; h < A + K; h++) {
	...
	if (whileIter == 1) {
		...
		//endTemp = clock();
		//checkTime = (double)(endImprove - startImprove) / (double)(CLOCKS_PER_SEC);
		...
		while (iterLP < 1 && (fabs((UBlp - OBJlp) / UBlp) >= 0.005) && checkTimeImprove < TimeLimitation2 && (NOimprovement < 3)) {// || imp_it < 1)) {
		...
		if (flag[sglobal] == 1){
			...
			for (h = 0; h < A + K; h++){
				matind[index] = pos_w[h];
				matval[index++] =c[h] * (alphaVio[(h)]- varphiVio[h]);
			}
	...
	/// For removing the optimality cuts with high slack presented in Section 5.4.3. of the paper
...
static int CPXPUBLIC lazy_feas_callback1(CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle, int *useraction_p)
{
	...
	d_vector(&temp_x, cur_numcols, "open_cplex:0");
																//Create the variable that will be used to store the solution at the node
	objval = &obval;																							//Assign the memory slot to the pointer so that we may use it
	numcols = cur_numcols;	//Transfer the information on the number of variables which we obtained at the make lazy constraint callback
