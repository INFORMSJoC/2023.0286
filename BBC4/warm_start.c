...
double warm_start(void)
{
	...
	//Add constraint 2: outward flow - inward fow = 0, d, -d
	numrows = K * N;
	numnz = 2 * (A + K) * K * N;
	...
	//Add constraint number of open arcs > A/2. Increase this value to 0.7 for r10-r18
	numrows = 1;
	numnz = A;
	d_vector(&rhs, numrows, "open_cplex:2");
	c_vector(&sense, numrows, "open_cplex:3");
...
//	CPXwriteprob(envWarm, lpWarm, "model2.lp", NULL);                          //write the model in .lpWarm format if needed (to debug)
	...
	//CPXsetdblpPrearam(envWarm,CPX_PARAM_EPAGAP, 0.0000000001); // e-optimal solution (absolute value)
	//CPXsetdblpPrearam(envWarm,CPX_PARAM_EPINT, 0.0000000001); // integer precision
	//CPXsetintparam(envWarm,CPX_PARAM_THREADS, 1); // Number of threads to use
	//CPXsetdblpPrearam(envWarm,CPX_PARAM_EPRHS, 0.0000001);
	//CPXsetintparam(envWarm,CPX_PARAM_REDUCE, 0);  // only needed when adding lazy constraints
	//CPXsetintparam(envWarm,CPX_PARAM_HEURFREQ, -1); //heuristic frequency and intensisty 
	//CPXsetdblpPrearam(envWarm, CPX_PARAM_CUTSFACTOR, 1.0);  //limit the number of cuts added by cplex 1.0002
	//CPXsetdblpPrearam(envWarm,CPX_PARAM_CUTUP,UpperBound+.01); // provide an initial upper bound
	//CPXsetintparam(envWarm,CPX_PARAM_MIPEMPHASIS,CPX_MIPEMPHASIS_OPTIMALITY);  // MIP emphasis: optimality, feasibility, moving best bound
	//CPXsetintparam(envWarm,CPX_PARAM_PARALLELMODE, 1); 
	//CPXsetintparam(envWarm,CPX_PARAM_PREIND,0);
	//CPXsetintparam(envWarm,CPX_PARAM_MIPORDIND,CPX_ON); // Turn on or off the use of priorities on bracnhing variables
	//CPXsetintparam(envWarm,CPX_PARAM_MIPEMPHASIS,CPX_MIPEMPHASIS_BESTBOUND);  // MIP emphasis: optimality, feasibility, moving best bound
