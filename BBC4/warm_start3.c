...
double warm_start3(void)
{
	...
	/// The y and w values are from warm_start2() function. We cannot use the (int)(A * 0.2) arcs with the highest flow. 
	counter = 0;
	for (h = 0; h < A; h++) {
		//w_hat[h] = w[h];
	...
	/*for (h = 0; h < A; h++) {
		printf("%d\t%.2f\t%d\t%d\n", h, Sorted[h].value, Sorted[h].index2, Sorted[h].index);
	}*/
