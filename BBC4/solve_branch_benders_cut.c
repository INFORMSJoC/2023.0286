...
double solve_branch_benders_cut(void)
{
	...
	for (ii = 0; ii < iter+1; ii++){
		...
		numCutRelax += index1;
		//status = CPXaddlazyconstraints(env, lp, 1, numnz, rhs, sense, matbeg, matind, matval, NULL);
	...
	//initnodeobjvalinfo(env, lp, &gcutinfo);
...
static int CPXPUBLIC lazy_feas_callback1(CPXCENVptr env, void *cbdata, int wherefrom, void *cbhandle, int *useraction_p)
{
	...
	double *sum, *BigI;
	sum = create_double_vector(A + K);
	BigI = create_double_vector(A + K);
